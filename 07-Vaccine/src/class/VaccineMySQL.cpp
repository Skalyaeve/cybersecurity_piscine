#include "../../include/class/VaccineMySQL.hpp"

// ===================================================================== //
// ============================ P U B L I C ============================ //
// ===================================================================== //

VaccineMySQL::VaccineMySQL() : VaccineBase() {}

VaccineMySQL::VaccineMySQL(
...
VaccineMySQL::VaccineMySQL(const VaccineMySQL& other)
    : VaccineBase(other) {}

VaccineMySQL::~VaccineMySQL() {}

VaccineMySQL& VaccineMySQL::operator=(const VaccineMySQL& other)
{
        if (this != &other)
...
// ==================================================== METHODS
void VaccineMySQL::stacked_queries()
{
}

void VaccineMySQL::union_based()
{
        if (!this->_data_check())
                return;
        std::string url;
        ...
        for (const auto& node : this->_gates)
        {
                ...
                std::cout << "[ INFO ] Testing " << url
                          << " with db type as MySQL ..." << std::endl;
                ...
                if (!this->_databases.empty())
                        ...
                        std::cout << "[ INFO ] Union based SQLi failed on "
                                  << url << " with db type as MySQL." << std::endl;
                offset.clear();
                payloads.clear();
                parameters.clear();
...
void VaccineMySQL::error_based()
{
}

void VaccineMySQL::blind_based()
{
}
...
// ==================================================== PREPARE
void VaccineMySQL::_fetch_databases(
...
void VaccineMySQL::_fetch_tables(
    const std::vector< std::string >& base_parameters,
    std::vector< std::string >& payloads,
    std::string& offset,
    const std::string& url,
...
void VaccineMySQL::_fetch_columns(
    const std::vector< std::string >& base_parameters,
    std::vector< std::string >& payloads,
    std::string& offset,
    const std::string& url,
...
void VaccineMySQL::_fetch_values(
    const std::vector< std::string >& base_parameters,
    std::vector< std::string >& payloads,
    std::string& offset,
    const std::string& url,
